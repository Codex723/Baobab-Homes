...
export function Footer() {
  ...
  return (
    <footer style={{ background: C.ink, borderTop: `1px solid rgba(245,242,236,0.08)`, padding: mob ? '40px 20px 24px' : '48px 40px 28px' }}>
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1.4fr 1fr 1fr 1fr 1fr', gap: mob ? 28 : 32, marginBottom: mob ? 28 : 36 }}>
          <div>
          ...
          {mob ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              {COLUMNS.map(([heading, links]) => <LinkColumn key={heading} heading={heading} links={links} />)}
            </div>
          ) : (
            COLUMNS.map(([heading, links]) => <LinkColumn key={heading} heading={heading} links={links} />)
          )}
